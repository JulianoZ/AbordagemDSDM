...
return args	
	...
	«" "»public «f.type.fullyQualifiedName»«IF !f.arrayAllow.equals(0)»[]«ENDIF» get«f.name.toFirstUpper»() {
	«" "»«" "»return «f.name»;
	«" "»}
	
	«" "»public void set«f.name.toFirstUpper»(«f.type.fullyQualifiedName»«IF !f.arrayAllow.equals(0)»[]«ENDIF» «f.name») {
	«" "»«" "»this.«f.name» = «f.name»;
	«" "»}
	«ENDIF»
	'''
